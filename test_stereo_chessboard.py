...
def test_match_by_projection(stereo, img_left, img_right, pts_2d_left_raw, pts_2d_right_raw, pts_3d_in_left,
    ...
    interested_pts_2d_left_img, des1, interested_pts_2d_right_img, des2 = \
        slam_lib.feature.get_sift_pts_and_sift_feats(img_left, img_right, shrink=3.5, flag_debug=False)
    ...
    interested_pts_2d_left_cam = stereo.cam_left.proj_pts_2d_img_frame_2_new_frame(interested_pts_2d_left_img)
    interested_pts_2d_right_cam = stereo.cam_left.proj_pts_2d_img_frame_2_new_frame(interested_pts_2d_right_img)

    # compute project line, which is interested pts on image
    left_interested_lines_3d_left_cam = np.concatenate(
        [np.copy(interested_pts_2d_left_cam), np.ones((interested_pts_2d_left_cam.shape[0], 1))],
        axis=-1) * 300
    right_interested_lines_3d_right_cam = np.concatenate(
        [np.copy(interested_pts_2d_right_cam), np.ones((interested_pts_2d_right_cam.shape[0], 1))],
        axis=-1) * 300
    ...
    interested_pts_2d_left_cam_match, *_ = cv2.projectPoints(
            interested_pts_3d_left_cam_match, rvec=cv2.Rodrigues(stereo.rotation_rectify_left)[0], tvec=np.zeros(3),
            cameraMatrix=stereo.camera_matrix_rectify_left[:3, :3], distCoeffs=None)
    interested_pts_2d_right_cam_match, *_ =  cv2.projectPoints(
            interested_pts_3d_right_cam_match, rvec=cv2.Rodrigues(stereo.rotation_rectify_right)[0], tvec=np.zeros(3),
    ...
    mask_epipolar = (interested_pts_2d_left_cam_match[:, 1] - interested_pts_2d_right_cam_match[:, 1]) < epipolar_error
    ...
    interested_pts_2d_left_img_match, interested_pts_2d_right_img_match = \
        interested_pts_2d_left_img[id_left_interested_pts_2_right_interested_pts[:, 0].tolist()], \
        interested_pts_2d_right_img[id_left_interested_pts_2_right_interested_pts[:, 1].tolist()]
